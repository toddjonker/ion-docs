...
// TODO Can we write attributes that expand to stem?  eg for domain names.
// This doesn't work; the stem isn't processed after the attribute substitution.
// :Dom_B:stem:[\DomainB]


== Introduction

The domain stem:[\DSName{Value}] is normal,
stem:[\DSName{Value*}] is seq,
stem:[\DSName{bad}] is bad.
...
which certain Ion encoding artifacts are consumed and/or transformed, ultimately
producing the user's data.
...
the syntax of an Ion document into data meaningful to some application.
To maintain this focus, we abstract over the details of how Ion parses the
wire-level octets, and instead treat as input the resulting "`parse tree`" or
abstract syntax tree.  Using the domain *Document* to denote these inputs, and
the domain *Value* to model a resulting application datum, we can formalize our
goal in the form of a function:

// TODO Generate these function signatures.
[{denotation}]
++++
\mbox{\it expand} : \mbox{\bf Document} \times \mbox{\bf Catalog} \rightarrow \mbox{\bf Value*}
++++
...
We now set upon the task of implementing that function by way of a formal
mathematical model.
...
* xref:syntaxmodel[xrefstyle=short] defines *Document* and related domains
representing the structure of Ion syntax, before expansions are performed.
...
And we can match the first annotation on a value, necessary to detect local
symbol table directives at top-level:
...
It is reasonable to think of these domains as representing parse trees coming
...
The data and syntax models are identical except for their handling of symbol
tokens, so the domains mirror each other.
Here we have the domain names prefixed with **R** to connote their raw nature.
...
address like `$4`.
Therefore, a *RSymTok* is either a Unicode string or an integer address:
...
Everything else in the syntax model simply propagates *RSymTok* in place of *SymTok*.
...
tables (LSTs).  The latter are simply annotated **RStruct**s that require no
...
And finally, we can model an entire Ion document:
...
process: replacing SIDs with text.  We introduce stem:[{\cal E}] ("`expand`") as
...
That requires a range check, returning `$0` if the SID exceeds the size of the
...
The atomic domains **NullNull**, **Bool**, **Int**, and **String** pass
through easily by being re-wrapped as **Content**.
...
Expanding a sequence is similar, with recursion on its sequence of elements:

[{denotation}]
++++
include::{modeltex}[tag=E_RList]
\newline\newline
include::{modeltex}[tag=E_RSexp]
++++
...
To start handling LST directives, we must first detect them by
...
The resulting environment after such a directive is as follows, where
...
This function sidesteps the more heavy-duty work of turning a symbol-table
...
First, recall that stem:[\odot] returns a sequence of zero or one values,
raising an error if the field is repeated in the struct.
When there's no `symbols` declared, or not an actual *List*, we return an empty
sequence.
...
will need to normalize its meaningful fields, checking presence, type, and
nullness.
...
We are down to our final denotations: the meaning of catalogs and the shared
